namespace ModularMonolith.Shared
{
    ...
    public class HttpTestClient : IDisposable
    {
        ...
        internal HttpTestClient(string routePrefix, HttpClient client)
        {
            this.client = client;
        ...
        internal static HttpTestClient CreateHttpApi<TEntryPoint>(string routePrefix, CustomWebApplicationFactory<TEntryPoint> factory, Action<string>? log) where TEntryPoint : class
        {
            ...
            return new HttpTestClient(routePrefix, client)
            {
                Log = log
            };
        }
